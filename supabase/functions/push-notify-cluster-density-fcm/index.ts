...
Deno.serve(async (req) => {
  try {
    ...
    const pyData: PythonResponse = await response.json()
    
    if (pyData.status !== 'success' || !pyData.data || pyData.data.length === 0) {
      return new Response(JSON.stringify({ message: 'Không có dữ liệu sự cố' }))
    }
    ...
    if (maxDensity < 3) {
      return new Response(JSON.stringify({ message: `Mật độ cao nhất chỉ là ${maxDensity}. Bỏ qua thông báo.` }))
    }
    ...
    if (tokens.length === 0) {
      return new Response(JSON.stringify({ message: 'Không có user nào có token' }))
    }
...
// ================= CÁC HÀM PHỤ TRỢ =================
async function saveNotificationsToDB(userIds: string[], title: string, body: string, clusterId: number) {
  const batchSize = 500
  for (let i = 0; i < userIds.length; i += batchSize) {
    ...
    const records = batch.map(userId => ({
      user_id: userId,
      body: body,
      type: 'cluster_alert', // Type mới phân biệt với 'incident'
      reference_id: null, // Không có reference_id cụ thể, (incident_id sẽ là null)
      cluster_id: clusterId, // Lưu Cluster ID 
      is_read: false
    }))
...
async function sendPushToMultiple(tokens: string[], title: string, body: string, clusterIdStr: string, accessToken: string) {
  const batchSize = 10
...
async function sendFCM(token: string, title: string, body: string, clusterIdStr: string, accessToken: string) {
  await fetch(`https://fcm.googleapis.com/v1/projects/${serviceAccount.project_id}/messages:send`, {
    ...
    body: JSON.stringify({
      message: {
        ...
        data: { type: 'cluster_alert', cluster_id: clusterIdStr }
      }
    })
  })
}
