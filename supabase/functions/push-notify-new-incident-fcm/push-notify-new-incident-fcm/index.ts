...
Deno.serve(async (req)=>{
  try {
    ...
    const tokens = recipients.map((u)=>u.fcm_token).filter((t)=>t !== null && t.trim().length > 0);
...
// ================= GET USERS =================
async function getRecipients(reporterUid) {
  const { data, error } = await supabase.from('users').select('id, fcm_token').neq('id', reporterUid);
  if (error) throw error;
  return data ?? [];
}
// ================= SAVE TO DB =================
async function saveNotificationsToDB(userIds, incident, body) {
...
// ================= SEND MULTIPLE =================
async function sendPushToMultiple(tokens, body, referenceId, accessToken) {
  ...
  for(let i = 0; i < tokens.length; i += batchSize){
    ...
    await Promise.all(batch.map((token)=>sendFCM(token, body, referenceId, accessToken)));
  }
}
// ================= SEND SINGLE =================
async function sendFCM(token, body, referenceId, accessToken) {
